...
public class InitMongoDBDataChangeLog {
    ...
    public void initBooks(BookRepository bookRepository) {
        books.add(bookRepository.save(new Book("Book_1", authors.get(0).block(), genres.get(0).block())));
        books.add(bookRepository.save(new Book("Book_2", authors.get(1).block(), genres.get(1).block())));
        books.add(bookRepository.save(new Book("Book_3", authors.get(2).block(), genres.get(2).block())));
    }
    ...
    public void initComments(CommentRepository commentRepository) {
        commentRepository.save(new Comment("Comment_1", books.get(0).block())).block();
        commentRepository.save(new Comment("Comment_2", books.get(1).block())).block();
        commentRepository.save(new Comment("Comment_3", books.get(2).block())).block();
    }
}
